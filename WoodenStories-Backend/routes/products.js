...
router.put("/:id", upload.array("images", 5), async (req, res) => {
  try {
    const { title, price, category, stock_quantity, description, existingImages } = req.body;

    // Parse existing image URLs coming from frontend (may be undefined)
    let existing = [];
    if (typeof existingImages === "string" && existingImages.length > 0) {
      try {
    ...
    const newImages = hasNewImages ? req.files.map((f) => f.path) : [];

    const shouldUpdateImagesColumn = hasNewImages || typeof existingImages !== "undefined";
    const finalImages = hasNewImages || typeof existingImages !== "undefined"
      ? [...existing, ...newImages]
      : null;

    if (shouldUpdateImagesColumn) {
      await pool.query(
      ...
      // No change requested for images: only update other fields
      await pool.query(
