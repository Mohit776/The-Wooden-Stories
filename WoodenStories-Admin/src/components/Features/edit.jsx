...
export default function EditProduct() {
  ...
  const [images, setImages] = useState([]); // newly selected files
  const [preview, setPreview] = useState([]); // previews for newly selected files
  const [existingImages, setExistingImages] = useState([]); // already saved image URLs
      ...
      try {
        ...
        const parsedExisting =
          typeof product.images === "string"
            ...
            : product.images || [];
        setExistingImages(parsedExisting);
        setPreview([]);
      } catch (err) {
        console.error(err);
  ...
  const handleImageChange = (e) => {
    ...
    const totalCount = files.length + existingImages.length + preview.length;
    if (totalCount > 5) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // send remaining existing image URLs so backend can keep only these
            ...
            <div>
              ...
              {(existingImages.length > 0 || preview.length > 0) && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-4">
                  ...
                  {existingImages.map((img, idx) => (
                    <div key={`existing-${idx}`} className="relative group">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 w-6 h-6 flex items-center justify-center rounded-full bg-red-600 text-white text-xs font-bold shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                        title="Remove image"
                      >
                        ×
                      </button>
                  ...
                  {/* New image previews (local selections) */}
                  {preview.map((img, idx) => (
              ...
              <div className="border-2 border-dashed border-[#d6c088] rounded-lg p-8 text-center hover:border-[#654f44] transition-colors duration-300">
                ...
                <p className="text-[#654f44] text-sm mb-4">
                  {existingImages.length + preview.length > 0
                    ? `${existingImages.length + preview.length} image(s) selected - ${5 - (existingImages.length + preview.length)} more allowed`
