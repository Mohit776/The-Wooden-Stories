...
export default function Dashboard() {
  ...
  const totalRevenue = orders.reduce((sum, it) => sum + (Number(it.amount) || 0), 0)
  const totalOrders = orders.length
  const totalProducts = products.length
  const totalCustomers = customers.length

  const stats = [
    { label: 'Total Revenue', value: `₹${totalRevenue.toLocaleString('en-IN')}`, change: '', trend: 'up', icon: DollarSign },
